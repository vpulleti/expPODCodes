...
kx = 2*np.pi*np.linspace(0,np.size(ufluc[:,10,10]),np.size(ufluc[:,10,10]))*1./(x[1:,10]-x[:-1,10])[10]
...
for tstp in range(num_tstp):
  uhat_fluc[:,:,tstp] = np.fft.fft(ufluc[:,:,tstp],n=1024,axis=0)/len(ufluc[:,10,tstp])
  vhat_fluc[:,:,tstp] = np.fft.fft(vfluc[:,:,tstp],n=1024,axis=0)/len(vfluc[:,10,tstp])

uhat_fluc = uhat_fluc[range(int(len(uhat_fluc[:,10,10])/2)),:,:]
vhat_fluc = vhat_fluc[range(int(len(vhat_fluc[:,10,10])/2)),:,:]

kx = 2*np.pi*np.linspace(0,np.size(uhat_fluc[:,10,10]),np.size(uhat_fluc[:,10,10]))*1./(x[1:,10]-x[:-1,10])[10]
